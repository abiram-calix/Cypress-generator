...
function createTestCode(JSONTestObject) {
  console.log("Generating test code for:", JSONTestObject.name);
  let codeBlock = "";
  ...
  JSONTestObject.steps.forEach((step) => {
    ...
    switch (action) {
      ...
      case "type":
        // Generate different code based on the actual step value and target element
        if (step.value === "" || step.value == null || step.value === "null") {
          // Empty value - just clear the field
  ...
  if (req.method === "POST" && req.url === "/generate-test") {
    ...
    req.on("end", async () => {
      try {
        ...
        const cypressTestFileName = jsonData.tests[0].name
          .replace(/\s+/g, "_")
          .toLowerCase();
        // console.log(body);
        ...
        // Run Cypress with the actual generated filename
        exec(
          `cd ../cypress && npx cypress run --config-file cypress.config.js --spec "cypress/e2e/generated/${cypressTestFileName}.cy.js"`,
          (error, stdout, stderr) => {
            if (error) {
